...
db.define_table('resource',
    ...
    Field('votes_user', 'list:reference auth_user', default=[], label=T('Ids user'), writable=False, readable=False),
...
db.resource.coverpag.requires = [IS_UPLOAD_FILENAME(filename='^(\s|\.|-|\w|á|é|í|ó|ú|ñ|Á|É|Í|Ó|Ú|Ñ){1,40}$', extension='^(png)$', case=0)]
db.resource.year.requires = [IS_NOT_EMPTY()]
db.resource.resource.comment = T("Filename length (40), alphanumeric characters and format (pdf, epub or png)")
db.resource.coverpag.comment = T("Filename length (40), alphanumeric characters, image format (png) and  (210x260)")
db.resource.resource.represent = lambda value, register: A(T('Click here for download'), _href=URL('resource', 'download.html', args=[register.id, value]))

db.resource.coverpag.requires = IS_EMPTY_OR(IS_IMAGE(extensions=('png'), maxsize=(210, 260)))

db.resource.title.requires = [IS_NOT_EMPTY()]
...
if ("Administrador" in auth.user_groups.values()) or ("Publicador" in auth.user_groups.values()):
    ...
    subconjunto=db((db.state.name != "Publicar"))
else:
    subconjunto=db((db.state.name != "Publicar") & (db.state.name != "Rechazar") & (db.state.name != "Aceptar"))
...
db.define_table('review',
   ...
   Field('success', 'boolean', default=False, writable=False, readable=False, label=T("Success")),
